...
from fairseq.data import DocNoiseDataset, LanguagePairDataset
...
class DocTranslationTask(TranslationTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--load-partial-global-from-local",
            action="store_true",
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        for key in attn_keys:
            ...
            metrics.log_scalar(
                key, val_sum / nsentences / math.log(2), nsentences, round=3
            )
        ...
        if split in ["train"]:
            if self.args.doc_noise_mask > 0:
                dataset = DocNoiseDataset(
                    dataset,
                    seed=self.args.seed,
                    naddition=self.naddition,
                    doc_noise_mask=self.args.doc_noise_mask,
                    doc_noise_epochs=self.args.doc_noise_epochs,
                )
